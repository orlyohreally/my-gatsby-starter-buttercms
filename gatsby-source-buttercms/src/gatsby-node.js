...
exports.sourceNodes = async ({
  ...
  // Support webhook events to deteNode and createNode
  // console.log(JSON.stringify(webhookBody, null, 2), !!webhookBody.webhook)

  if(webhookBody && webhookBody.webhook) {
    console.log("webhook");
    ...
    const { webhook, data: item} = webhookBody;
    reporter.verbose(`Handling webhook: ${webhook.event}`);
    switch (webhook.event) {
      case "post.delete":
      case "page.delete":
      case "collectionitem.delete":
        reporter.verbose(`Deleting Item ID: ${item.id}`)
        deleteNode({
          node: getNode(createNodeId(item.id)),
        })
        break;
      case "post.draft":
      case "post.published":
      case "page.draft":
      case "page.published":
      case "collectionitem.draft":
      case "collectionitem.published":
        reporter.verbose(`Creating/updating node based on event for object ID: ${item.id}`);
